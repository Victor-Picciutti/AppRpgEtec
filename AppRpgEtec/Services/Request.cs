...
namespace AppRpgEtec.Services
{
    public class Request
    {
        ...
        public async Task<int> PutAsync<TResult>(string uri, TResult data, string token)
        {
            ...
            httpsClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

            var content = new StringContent(JsonConvert.SerializeObject(data));
            content.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            HttpResponseMessage response = await httpsClient.PutAsync(uri, content);
